...
def ask(question: str):
    ...
    for candi in ordered_candidates:
        ctx_list.append(sources[candi[1]])
        sim_list.append(candi[0])
    ...
    return [prompt, completion.choices[0].message.content,ctx_list,sim_list]
...
def process_text(input_text):
    ...
    for i in range(len(result[2])):
        res_str += str(result[3][i])+"   "+result[2][i] + "\n"
...
#     outputs=gr.outputs.Textbox(label="Processed Text"),
